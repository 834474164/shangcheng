<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:65:"D:\www\shangcheng\public/../application/admin\view\auth\edit.html";i:1532699538;s:52:"D:\www\shangcheng\application\admin\view\layout.html";i:1532697794;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
<div class="sidebar-nav">
    ...
    <div class="content">
        ...
        <div class="well">
            <!-- add form -->
            <form id="tab" action="<?php echo url('update'); ?>" method="post">
                <input type="hidden" name="id" value="<?php echo $authdata['id']; ?>">
                <label>权限名称：</label>
                <input type="text" name="auth_name" value="<?php echo $authdata['auth_name']; ?>" class="input-xlarge">
                <label>上级权限</label>
                <select pid="<?php echo $authdata['pid']; ?>" name="pid" class="input-xlarge">
                    <option value="0">作为顶级权限</option>
                    <?php foreach($topauth  as $v): ?>
                    <option  value="<?php echo $v['id']; ?>" <?php if(($v['id'] == $authdata['pid'])): ?>selected="selected"<?php endif; ?>><?php echo $v['auth_name']; ?></option>
                    <?php endforeach; ?>
                </select>
                <div>
                    <label>控制器：</label>
                    <input type="text" name="auth_c" value="<?php echo $authdata['auth_c']; ?>" class="input-xlarge">
                </div>
                <div>
                    <label>方法：</label>
                    <input type="text" name="auth_a" value="<?php echo $authdata['auth_a']; ?>" class="input-xlarge">
                </div>
                <label>是否菜单项</label>
                <select name="is_nav" class="input-xlarge">
                    <option value="1">是</option>
                    <option value="0" <?php if(($authdata['is_nav'] == '否')): ?>selected="selected"<?php endif; ?>>否</option>
                </select>
